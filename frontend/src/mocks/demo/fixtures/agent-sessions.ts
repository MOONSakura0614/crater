...
export const DEMO_AGENT_SESSIONS: DemoSessionBundle[] = [
  buildBundle({
    ...
    tools: [
      { name: 'query_queue_status', args: { queue: 'gpu-l40s' }, result: { used: 16, capacity: 16, pending: 12 }, role: 'explorer' },
      { name: 'query_user_quota', args: { account: 'nlp-lab' }, result: { gpuUsed: 6, gpuLimit: 8 }, role: 'explorer' },
      { name: 'query_cluster_capacity', args: { gpu_type: 'l40s' }, result: { ready: 2, maintenance: 2 }, role: 'explorer' },
      { name: 'query_recent_scheduler_events', args: { job: 'my-train-job' }, result: { failed_count: 8, reasons: ['InsufficientGPU'] }, role: 'explorer' },
    ],
    events: [
      ...
      { role: 'verifier', type: 'agent_handoff', summary: '证据完整，结论可支撑', verification: 'pass' },
  ...
  buildBundle({
    ...
    tools: [
      { name: 'get_job_detail', args: { name: 'nlp-train-001' }, result: { status: 'Failed', exitCode: 137 }, role: 'explorer' },
      { name: 'get_job_events', args: { name: 'nlp-train-001' }, result: { events: ['OOMKilled', 'GPU mem peak 23.6/24'] }, role: 'explorer' },
      { name: 'get_container_logs', args: { name: 'nlp-train-001', tail: 200 }, result: { tail: 'CUDA out of memory' }, role: 'explorer' },
      { name: 'query_job_metrics', args: { name: 'nlp-train-001', metrics: ['gpu_memory'] }, result: { peak: 23.6, avg: 21.4 }, role: 'explorer' },
      { name: 'query_similar_failures', args: { reason: 'OOMKilled' }, result: { count: 3, pattern: 'batch=64 + 16G' }, role: 'explorer' },
    ],
    events: [
      ...
      { role: 'explorer', type: 'agent_handoff', summary: 'handoff to verifier' },
      { role: 'verifier', type: 'agent_handoff', summary: '三重证据互证：OOMKilled / 峰值 23.6G / 历史模式', verification: 'pass' },
  ...
  buildBundle({
    ...
    assistantMessage:
      '作业 stuck-job-7 已停止，释放 4 GPU。未发现联动作业受影响。审计已记录。',
    tools: [
      { name: 'get_job_detail', args: { name: 'stuck-job-7' }, result: { status: 'Running', gpus: 4 }, role: 'explorer' },
      { name: 'check_job_dependents', args: { name: 'stuck-job-7' }, result: { dependents: [] }, role: 'explorer' },
      { name: 'stop_job', args: { name: 'stuck-job-7', grace_period_seconds: 30 }, result: { ok: true }, role: 'executor' },
      { name: 'get_job_detail', args: { name: 'stuck-job-7' }, result: { status: 'Stopped' }, role: 'verifier' },
    ],
    events: [
      { role: 'coordinator', type: 'agent_run_started', summary: '识别为受控执行任务（high risk）' },
      { role: 'coordinator', type: 'agent_handoff', summary: 'handoff to planner' },
      ...
      { role: 'executor', type: 'tool_call_confirmation_required', summary: '等待用户确认 stop_job' },
      { role: 'executor', type: 'tool_call', summary: 'stop_job 执行完成' },
      { role: 'verifier', type: 'tool_call', summary: 'get_job_detail 复核 Stopped 状态' },
      { role: 'verifier', type: 'agent_handoff', summary: '操作结果与意图一致', verification: 'pass' },
  ...
  buildBundle({
    ...
    tools: [
      { name: 'get_cluster_health', args: {}, result: { total: 318, failureRate: 0.085 }, role: 'explorer' },
      { name: 'list_nodes', args: {}, result: { degraded: ['gpu-node-03'] }, role: 'explorer' },
      { name: 'query_queue_status', args: { all: true }, result: { 'gpu-l40s': '16/16+12pending' }, role: 'explorer' },
      { name: 'detect_idle_jobs', args: { gpu_util_threshold: 0.05, min_duration_hours: 2 }, result: { count: 7 }, role: 'explorer' },
      { name: 'query_recent_failures', args: { window_hours: 24 }, result: { count: 27, top: 'OOM=11' }, role: 'explorer' },
    ],
    events: [
      { role: 'coordinator', type: 'agent_run_started', summary: '识别为巡检流水线任务' },
      { role: 'coordinator', type: 'agent_handoff', summary: 'handoff to planner' },
      { role: 'planner', type: 'agent_status', summary: '5 步巡检：健康 / 节点 / 队列 / 空跑 / 失败' },
      ...
      { role: 'verifier', type: 'agent_handoff', summary: '证据覆盖 5 个维度', verification: 'pass' },
      { role: 'coordinator', type: 'pipeline_report', summary: '巡检报告：3 个 category 9 个 item' },
  ...
  buildBundle({
    ...
    tools: [
      { name: 'get_node_detail', args: { name: 'gpu-node-03' }, result: { ready: true, gpuHealthy: false }, role: 'explorer' },
      { name: 'list_jobs_on_node', args: { node: 'gpu-node-03' }, result: { jobs: ['cv-train-12', 'rl-train-04', 'big-job-1'] }, role: 'explorer' },
      { name: 'get_node_events', args: { name: 'gpu-node-03' }, result: { xid: 4 }, role: 'explorer' },
      { name: 'query_prometheus', args: { query: 'DCGM_FI_DEV_XID_ERRORS{Hostname="gpu-node-03"}' }, result: { count: 4 }, role: 'explorer' },
    ],
    events: [
      ...
      { role: 'verifier', type: 'agent_handoff', summary: '4 个独立来源互证：硬件故障', verification: 'pass' },
  ...
  buildBundle({
    ...
    assistantMessage:
      '已停止 5 个空跑作业，释放 5 GPU，预计回收 17.5 GPU·h。已通知对应用户邮箱。',
    tools: [
      { name: 'detect_idle_jobs', args: { gpu_util_threshold: 0.05, min_duration_hours: 2 }, result: { count: 7 }, role: 'explorer' },
    ...
    events: [
      { role: 'coordinator', type: 'agent_run_started', summary: '识别为批量受控执行任务（high risk）' },
